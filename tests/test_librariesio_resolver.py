...
class TestResolveUnknownType:
    @pytest.mark.asyncio
    async def test_unknown_purl_type_returns_warning(self) -> None:
        r = LibrariesIoResolver(api_key="test_key")
        result = await r.resolve("pkg:deb/debian/libssl")
...
class TestResolveNoRepository:
    @pytest.mark.asyncio
    async def test_no_repository_in_response(self) -> None:
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "name": "some-package",
        ...
        result = await r.resolve("pkg:pypi/some-package")
        assert result.repository_url is None
        assert any("no repository" in w.lower() for w in result.warnings)

    @pytest.mark.asyncio
    async def test_cran_package_without_repository(self) -> None:
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "name": "sleev",
        ...
        result = await r.resolve("pkg:cran/sleev@1.1.1")
        assert result.repository_url is None
        assert any("no repository" in w.lower() for w in result.warnings)
...
class TestResolveErrors:
    ...
    @pytest.mark.asyncio
    async def test_429_returns_warning(self) -> None:
        mock_response = MagicMock()
...
class TestRateLimiting:
    ...
    async def test_minimum_interval_between_requests(self) -> None:
        ...
        await r.resolve("pkg:pypi/requests")
        await r.resolve("pkg:npm/express")
