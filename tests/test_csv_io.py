...
class TestParseCsvImport:
    ...
    def test_resolver_column_parsed(self) -> None:
        csv_text = (
            "purl,repository_url,resolver\n"
            "pkg:pypi/test,https://example.com,custom\n"
        )
        rows, errors = parse_csv_import(csv_text)
        assert len(rows) == 1
        assert rows[0].resolver == "custom"

    def test_default_resolver_when_omitted(self) -> None:
        csv_text = (
            "purl,repository_url\n"
        ...
        rows, errors = parse_csv_import(csv_text)
        assert len(rows) == 1
        assert rows[0].resolver == "import-csv"
