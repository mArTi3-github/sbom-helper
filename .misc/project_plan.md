...
- **удаление компонентов без ссылок из SBOM** — удаление элементов списка components, у которых не получилось найти ссылку на исходные тексты и у которых при этом собственных списков components, из обогащаемого SBOM-файла
- **Раздел настроек** — добавление раздела настроек, в котором как минимум должны указываться API-ключи для взаимодействия с сервисами, требующими такие ключи, а также указание набора используемых резолверов
...
- **Валидация данных в БД** — валидация сопоставления PURL и repository URL при использовании данных из локальной БД с помощью механизма `git ls-remote` и актуализация/удаление невалидных записей
