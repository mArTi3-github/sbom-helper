...
При обработке SBOM-файлов (а также при выполнении других запросов к API) выполнение параллельных запросов через веб-интерфейс блокируется или происходит с большой задержкой. Причина: синхронные вызовы внутри async функций блокируют event loop FastAPI.
